...
def handle_ride_request_accept(data, conn):
    try:
        _, driver_id, request_id, acceptance_time = data.split("|")
        ...
        if not validator.validate_user_id(request_id):
            return "ERROR|Invalid request_id"
        if not validator.validate_timestamp(acceptance_time):
        ...
        if not is_locked:
            return "ERROR|Lock not acquired"
        if int(driver_id) != int(accepted_driver_id):
            return f"RIDE_ALREADY_TAKEN|{request_id}|Driver {accepted_driver_id}"

        cursor.execute("""
            UPDATE ride_requests
            SET status='accepted', acceptance_time=?
            WHERE request_id=?
        """, (acceptance_time, request_id))

        conn.commit()
        return "SUCCESS|Ride request accepted"
    except Exception as e:
...
#This gives passengers real-time tracking by always fetching the newest GPS point.
def handle_ride_location_latest(data, conn):
    """
    Fetch the most recent GPS location for a given ride request.
    Format: RIDE_LOCATION_LATEST|request_id
    """
    try:
...
def handle_ride_location_history(data, conn):
    """
    Fetch all GPS location points for a given ride request.
    Format: RIDE_LOCATION_HISTORY|request_id
    """
    try:
        # Split and validate input
        parts = data.split("|")
        ...
        # Format each location point into a readable string
        history = ";".join([
